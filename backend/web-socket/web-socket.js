            ...
            try {
                ...
                connections.forEach((client) => {
                    if (client.readyState === WebSocket.OPEN) {
                        client.send(JSON.stringify({ type: 'joined', gameStatus: currGame.status, players:currGame.players }));
                    }
                });
                // Listen for messages
                        ...
                        if (msg.type === 'move') {
                            ...
                            connections.forEach((client) => {
                                if (client.readyState === WebSocket.OPEN && client !== ws) {
                                    client.send(JSON.stringify({ type: 'move', payload: move, gameStatefen: currGame.gameStatefen }));
                                }
                            });
                        }
                            ...
                            connections.forEach((client) => {
                                if (client.readyState === WebSocket.OPEN) {
                                    client.send(JSON.stringify({ type: 'joined', gameStatus: currGame.status }));
                                }
                            });
                        }
                            ...
                            currGame.chat = [...currGame.chat, { name: req.user.name, text: msg.payload }];
                            await currGame.save();
                            // Broadcast the move to all other clients
                            const connections = games.get(gameID);
                            connections.forEach((client) => {
                                if (client.readyState === WebSocket.OPEN) {
                                    client.send(JSON.stringify({ type: 'chat', payload: { name: req.user.name, text: msg.payload } }));
                                }
                            });
                        }
                    } catch (err) {
